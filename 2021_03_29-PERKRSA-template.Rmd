...
vignette: >
  %\VignetteIndexEntry{KRSA Vignette}
...
###JFC_All you need to do is change these values

#Define path for input files
JFC_directory_input_Median_SigmBg <- "jfc_input/Brunicardi/03_PTK_drugs/PANC1_met-sim-dig_PTK/DataExports/PANC1_C3_PTK-KRSA/_ExportstosKRSA_Median_SigmBg_210318162301.txt"
JFC_directory_input_Signal_Saturation <- "jfc_input/Brunicardi/03_PTK_drugs/PANC1_met-sim-dig_PTK/DataExports/PANC1_C3_PTK-KRSA/_ExportstosKRSA_Signal_Saturation_210318162303.txt"


#Define Experiment ID
JFC_experiment_ID <- "c3-1-met"

#Define Comparison variables
JFC_comparand_a_as_it_appears_in_input <- "e_Met"
JFC_comparand_a_shorthand <- "e_met"

JFC_comparand_b_as_it_appears_in_input <- "c_Control"
JFC_comparand_b_shorthand <- "c_ctrl"

#Define chip variable: PTK, STK, PTK-single, STK-single. Single here uses per-kinase mapping instead of per-family mapping
JFC_chip_value <- "PTK"

#Define sample column name
JFC_sample <- "SampleName"

#Define group column name
JFC_group <- "Comment 1"

#define kinases of interest
JFC_kinases_of_interest <- c("Arg","BLK","DDR1","EphA8","FRK","Fyn","HCK","Lck","Lyn","TEC","EGFR","EphA2","FLT4","InSR","PDGFR[alpha]","Src","Ack1")



############################################################
#Variables under this line will likely remain constant because they are dynamically generated based on variables above

#Define identification variables
JFC_comparison <- paste0(JFC_comparand_a_shorthand,"-v-",JFC_comparand_b_shorthand)


#Define Directories
JFC_directory_output <- "jfc_output/"
JFC_directory_output_experiment_ID <- paste0(JFC_directory_output,JFC_experiment_ID,"/")
JFC_directory_output_whole <- paste0(JFC_directory_output_experiment_ID,"whole","/")
JFC_directory_output_comparison <- paste0(JFC_directory_output_experiment_ID,JFC_comparison,"/")
JFC_directory_output_comparison_per <- paste0(JFC_directory_output_experiment_ID,JFC_comparison,"/","per","/")
  
#Directories; dynamically generated; you shouldn't need to change these at all

#create output directory
if (dir.exists(JFC_directory_output)==F){dir.create(JFC_directory_output)}
if (dir.exists(JFC_directory_output_experiment_ID)==F){dir.create(JFC_directory_output_experiment_ID)}
if (dir.exists(JFC_directory_output_whole)==F){dir.create(JFC_directory_output_whole)}
if (dir.exists(JFC_directory_output_comparison)==F){dir.create(JFC_directory_output_comparison)}
if (dir.exists(JFC_directory_output_comparison_per)==F){dir.create(JFC_directory_output_comparison_per)}
...
knitr::include_graphics("vignettes/images/pamgene_workflow.png")
knitr::include_graphics("vignettes/images/pamgene_detectionFig.png")
```
...
```{r design, echo=F, fig.width=3, fig.height=3, fig.align="center", cache=TRUE}
tt <- tibble(
...
# Define chip type
chipType <- JFC_chip_value

# Read crosstab view bionavigator files and tidy data
data <- krsa_read(JFC_directory_input_Median_SigmBg, JFC_directory_input_Signal_Saturation)

#Rename sample and group columns in input data
names(data)[names(data) == paste0(JFC_group)] <- "Group"
...
#original (this no longer needs to be run for JFC data)
...
-   **JFC_comparand_a vs JFC_comparand_b**
...
comparisons <- list(Comp1 = c(paste0(JFC_comparand_a_as_it_appears_in_input), paste0(JFC_comparand_b_as_it_appears_in_input)))
...
#Save LFC table
diff_df %>% write_delim(paste0(JFC_directory_output_comparison_per,Sys.Date(),"-","LCF_df.txt"), delim = "\t")
...
uka_mp <- readRDS("data-raw/uka_map_db.RDS")
...
### JFC_comparand_a vs JFC_comparand_b (Heatmap)
...
#Khaled, this chunk does not need to be changed from KRSA to PERKRSA, correct? In fact, it can be deleted because it is already handled by KRSA.
...
### JFC_comparand_a vs JFC_comparand_b (Violin Plot)
...
### JFC_comparand_a vs JFC_comparand_b (Waterfall Plot)
...
#Khaled, this line does not need to be changed from KRSA to PERKRSA, correct? In fact, it can be deleted because it is already handled by KRSA.
...
#PERKRSA, this is great. Thanks Khaled.
# waterfall plot 
mapped_lfc_table %>% 
  # choose kinase to plot
  filter(Kin %in% JFC_kinases_of_interest[1:2]) %>% 
  krsa_waterfall(0.2) +
  facet_wrap(~Kin, scales = "free_y")

#PERKRSA, this is also great. Thank you
...
### JFC_comparand_a vs JFC_comparand_b (Curve Plot)
...
### JFC_comparand_a vs JFC_comparand_b (Upstream Kinase Analysis)
...
# to save file
fin$KRSA_Table %>% write_delim(paste0(JFC_directory_output_comparison_per,Sys.Date(),"-","acrossChip_KRSA_FullTable_comp1.txt"), delim = "\t")
...
# save file
AvgZTable %>% write_delim(paste0(JFC_directory_output_comparison_per,Sys.Date(),"-","acrossChip_KRSA_Table_comp1.txt"), delim = "\t")
...
# save file
AvgZTable %>% write_delim(paste0(JFC_directory_output_comparison_per,Sys.Date(),"-","withinChip_KRSA_Table_comp1.txt"), delim = "\t")
...
### JFC_comparand_a vs JFC_comparand_b (Z Scores Plot)
...
#Khaled, can we make a Z scores waterfall plot per individual kinase rather than kinase family?
...
### JFC_comparand_a vs JFC_comparand_b (Reverse KRSA Plot)
...
#Khaled, are the changes below sufficient and accurate? I used your per_peptide.Rmd as a model?
...
# plot the reverse KRSA figure for top kinases to determine their activity levels
#PERKRSA
krsa_reverse_krsa_plot(processed_uka_map, diff_df, kinases = JFC_kinases_of_interest, 0.2, byChip = F)
...
### JFC_comparand_a vs JFC_comparand_b (Coverage Plot)
...
#khaled, can this plot present PERKRSA info?

# generates a kinase coverage plot 
krsa_coverage_plot(processed_uka_map, diff_df, chipType)

```

\newpage

### JFC_comparand_a vs JFC_comparand_b (Ball Model Network)
...
#Khaled, can this be plotted on a per-kinase basis?

# Plot the network ball model
krsa_ball_model(JFC_kinases_of_interest, AvgZTable2, 10, 2.5, 4.8)
