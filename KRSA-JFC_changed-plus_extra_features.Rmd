...
JFC_experiment_ID <- "June16_PTK"


#Define Comparison variables
JFC_comparand_a_as_it_appears_in_input <- "e_human_liver_fibrosis"
JFC_comparand_a_shorthand <- "h_f"

JFC_comparand_b_as_it_appears_in_input <- "c_human_liver_normal"
JFC_comparand_b_shorthand <- "h_c"
...
JFC_directory_input_Median_SigmBg <- "jfc_input/June16_PTK-KRSA/_ExportstosKRSA_Median_SigmBg_210305150653.txt"
JFC_directory_input_Signal_Saturation <- "jfc_input/June16_PTK-KRSA/_ExportstosKRSA_Signal_Saturation_210305150655.txt"
...
JFC_directory_output_comparison_single <- paste0(JFC_directory_output_experiment_ID,JFC_comparison,"/","single","/")
...
if (dir.exists(JFC_directory_output_comparison_single)==F){dir.create(JFC_directory_output_comparison_single)}
...
#Save LFC table
diff_df %>% write_delim(paste0(JFC_directory_output_comparison,Sys.Date(),"-","LCF_df.txt"), delim = "\t")
...
if(chipType == "STK-single"){chipCov <- KRSA_coverage_STK_PamChip_87102_v1}
if(chipType == "STK-single"){KRSA_file <- KRSA_Mapping_STK_PamChip_87102_v1}
if(chipType == "PTK-single"){chipCov <- KRSA_coverage_PTK_PamChip_86402_v1}
if(chipType == "PTK-single"){KRSA_file <- KRSA_Mapping_PTK_PamChip_86402_v1}
...
# to save file
fin$KRSA_Table %>% write_delim(paste0(JFC_directory_output_comparison,Sys.Date(),"-","acrossChip_KRSA_FullTable_comp1.txt"), delim = "\t")
...
# save file
AvgZTable %>% write_delim(paste0(JFC_directory_output_comparison,Sys.Date(),"-","acrossChip_KRSA_Table_comp1.txt"), delim = "\t")
...
# save file
AvgZTable %>% write_delim(paste0(JFC_directory_output_comparison,Sys.Date(),"-","withinChip_KRSA_Table_comp1.txt"), delim = "\t")
...
uka_mp %>% distinct(.keep_all = T) %>% 
  ...
  filter(family == (paste0(JFC_chip_value))) %>% 
...
JFC_kinases_of_interest <- c("PKC[delta]","PKC[alpha]","PKA[alpha]","PKC[eta]","PKC[theta]","PKC[gamma]","PKC[beta]","PKG1","PKC[epsilon]","PKC[zeta]","Pim2","PKC[iota]","SGK2","Pim1","ROCK2","DAPK2","PKG2","PRKX","Pim3","Akt2/PKB[beta]","p70S6K[beta]","CK2[alpha]1","Akt1/PKB[alpha]","AMPK[alpha]1","CDK10")
...
#cmgc
JFC_kinases_of_interest_group1 <- c("CK2[alpha]1","CDK10")

#camk
JFC_kinases_of_interest_group2 <- c("Pim1","Pim2","Pim3","DAPK2")

#agc
JFC_kinases_of_interest_group3 <- c("PKC[delta]","PKC[alpha]","PKA[alpha]","PKC[eta]","PKC[theta]","PKC[gamma]","PKC[beta]","PKG1","PKC[epsilon]","PKC[zeta]","PKC[iota]","SGK2","ROCK2","PKG2","PRKX","Akt2/PKB[beta]","p70S6K[beta]","Akt1/PKB[alpha]","AMPK[alpha]1")
